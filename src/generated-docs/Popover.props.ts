/**
 * AUTO-GENERATED FILE. DO NOT EDIT.
 * Generated by scripts/generate-component-docs.cjs
 */

import type { GeneratedComponentDoc } from "./types";

export const popOverPropDocs: GeneratedComponentDoc = {
  name: "PopOver",
  interfaceName: "PopOverProps",
  description: "Props for the PopOver component.",
  sourcePath: "src\\components\\PopOver\\PopOver.types.ts",
  props: [
    {
      name: "aria-describedby",
      type: "string",
      description:
        "Associates the popover content with an external description element ID.",
      required: false,
      inherited: false,
      category: "aria",
    },
    {
      name: "aria-label",
      type: "string",
      description: "Accessible label for the popover content region.",
      required: false,
      inherited: false,
      category: "aria",
    },
    {
      name: "aria-labelledby",
      type: "string",
      description:
        "Associates the popover content with an external label element ID.",
      required: false,
      inherited: false,
      category: "aria",
    },
    {
      name: "aria-modal",
      type: "boolean",
      description:
        "Whether the popover should be announced as modal when role is dialog.",
      required: false,
      inherited: false,
      category: "aria",
    },
    {
      name: "asChild",
      type: "boolean",
      description:
        "When true, the popover will clone the provided trigger element and attach popover behavior directly to it instead of wrapping it in an internal button.",
      required: false,
      inherited: false,
      category: "props",
      defaultValue: "false",
    },
    {
      name: "className",
      type: "string",
      description: "Optional additional class name(s) for custom styling.",
      required: false,
      inherited: false,
      category: "styling",
    },
    {
      name: "content",
      type: "ReactNode",
      description: "Content to be displayed inside the popover.",
      required: true,
      inherited: false,
      category: "props",
    },
    {
      name: "contentClassName",
      type: "string",
      description:
        "Optional additional class name(s) for the popover content element.",
      required: false,
      inherited: false,
      category: "props",
    },
    {
      name: "data-testid",
      type: "string",
      description: "Backward-compatible alias for test ID attributes.",
      required: false,
      inherited: false,
      category: "testing",
    },
    {
      name: "disabled",
      type: "boolean",
      description: "Disables interaction with the trigger and popover.",
      required: false,
      inherited: false,
      category: "props",
      defaultValue: "false",
    },
    {
      name: "id",
      type: "string",
      description: "Optional ID for the popover content element.",
      required: false,
      inherited: false,
      category: "props",
    },
    {
      name: "placement",
      type: '"left" | "right" | "top" | "bottom"',
      description:
        'Placement of the popover relative to the trigger element. One of: "top" | "bottom" | "left" | "right"',
      required: false,
      inherited: false,
      category: "props",
      defaultValue: '"bottom"',
    },
    {
      name: "rounding",
      type: "RoundingType",
      description:
        'Rounding of the popover content. One of: "none" | "small" | "medium" | "large" | "full"',
      required: false,
      inherited: false,
      category: "styling",
      defaultValue: 'configured default rounding (fallback: "medium")',
    },
    {
      name: "shadow",
      type: "ShadowType",
      description:
        'Shadow of the popover content. One of: "none" | "light" | "medium" | "strong" | "intense"',
      required: false,
      inherited: false,
      category: "styling",
    },
    {
      name: "state",
      type: "StateType",
      description:
        'State of the popover, for feedback styling. One of: "success" | "error" | "warning" | "disabled" | ""',
      required: false,
      inherited: false,
      category: "styling",
    },
    {
      name: "testId",
      type: "string",
      description: "Optional test ID for testing frameworks.",
      required: false,
      inherited: false,
      category: "props",
      defaultValue: 'dataTestId ?? "popover"',
    },
    {
      name: "theme",
      type: "ThemeType",
      description:
        'Theme for the popover styling. One of: "primary" | "secondary" | "tertiary" | "quaternary" | "clear"',
      required: false,
      inherited: false,
      category: "styling",
      defaultValue: 'configured default theme (fallback: "primary")',
    },
    {
      name: "trigger",
      type: "ReactNode | React.ReactElement<TriggerElementProps, string | JSXElementConstructor<any>>",
      description:
        "Trigger content for the popover. When `asChild` is false or omitted, this can be any renderable node and will be wrapped in an internal button element. When `asChild` is true, this should be a single interactive React element such as a button, link, or custom Button component so the popover can attach behavior directly to it without nesting buttons.",
      required: true,
      inherited: false,
      category: "props",
    },
    {
      name: "triggerAriaLabel",
      type: "string",
      description:
        "Accessible label for the trigger button or trigger element.",
      required: false,
      inherited: false,
      category: "props",
    },
    {
      name: "triggerTitle",
      type: "string",
      description: "Optional title attribute for the trigger element.",
      required: false,
      inherited: false,
      category: "props",
    },
    {
      name: "variant",
      type: 'import("C:/Users/davin/Documents/Web Design/boreal-ui/src/index.core").VariantType',
      description:
        "Surface treatment; glassOutline combines glass and outline.",
      required: false,
      inherited: false,
      category: "props",
      defaultValue: "getDefaultVariant()",
    },
  ],
};
