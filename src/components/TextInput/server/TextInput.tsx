...
export default function TextInput({ label, labelPosition = "top", icon: Icon, placeholder = "Enter text", theme = getDefaultTheme(), glass = getDefaultGlass(), rounding = getDefaultRounding(), shadow, state, disabled = false, outline = getDefaultOutline(), className, containerClassName, labelClassName, iconClassName, inputClassName, testId, "data-testid": dataTestId, id, readOnly = true, ...rest }: ServerTextInputProps) {
  ...
  return <div className={combineClassNames(classMap.container, classMap[`label${capitalize(position)}`], containerClassName)} data-testid={resolvedTestId}>
    ...
    <div className={combineClassNames(classMap.textInput, classMap[theme], state && classMap[state], outline && classMap.outline, glass && classMap.glass, disabled && classMap.disabled, getShadowClassName(classMap, theme, shadow), rounding && classMap[`round${capitalize(rounding)}`], className)}>
